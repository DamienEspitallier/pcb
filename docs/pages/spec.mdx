...
6. [Schematic Export](#schematic-export)
7. [Schematic Position Comments](#schematic-position-comments)
...
`pcb sch <FILE>.zen` generates a human-readable, hierarchical KiCad schematic
(`.kicad_sch`) from a design. It evaluates the `.zen` file exactly like `pcb
build`/`pcb layout`, then places, wires, and labels the resulting netlist
procedurally.

```bash
pcb sch MyBoard.zen            # write MyBoard.kicad_sch (+ sub-sheets)
pcb sch MyBoard.zen --check    # also run KiCad ERC on the result
```

Key behavior:

- **Hierarchy** — each module instance becomes its own sheet, linked from its
  parent with a sheet symbol; trivial modules (only passives, or fewer than a
  few parts) are inlined instead of getting a sheet.
- **Placement** — components flow left-to-right by role; satellites
  (decoupling, pull-ups, bulk) cluster around their anchor IC; power rails go
  up and grounds go down. Wires are drawn directly where the topology is
  simple, and net labels are used otherwise.
- **Manual positions** — any `# pcb:sch` placement recorded in the `.zen`
  file (see below) is honored as a hard constraint; the auto-placer only
  positions the remaining components around it.
- **Deterministic** — two runs of the same design produce byte-identical
  files (all UUIDs are derived from the design, never random).

Options:

- `-o, --output <PATH>` — output file or directory. Defaults to the design's
  layout directory when it declares one (`Board(layout_path=...)`), otherwise
  a sibling of the `.zen` file.
- `--check` — run `kicad-cli` ERC on the generated schematic (errors are
  fatal, warnings are reported).
- `--config <KEY=VALUE>` — override root `config()` parameters, same as
  `pcb build`.
- `--offline` — only use vendored dependencies.

`pcb sch` writes (or patches) a KiCad project file (`.kicad_pro`) next to the
schematic so ERC does not flag the embedded symbol libraries; an existing
project file (e.g. one maintained by `pcb layout`) is preserved and only its
ERC severities are adjusted.
